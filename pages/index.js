...
const LBL = { upload:'Upload', transcribe:'Auto-Transcript', enhance:'Enhance', voice:'Voice', merge:'Merge', done:'Done' }
...
const Card = ({children,accent}) => <div style={{background:'#0a0a0a',border:`1px solid ${accent?'#0f2a1f':'#141414'}`,borderRadius:12,padding:18,marginBottom:14}}>{children}</div>
const Btn = ({onClick,disabled,loading,children,secondary}) => (
  <button onClick={onClick} disabled={disabled||loading} style={{
    width:'100%',padding:'14px',fontFamily:'inherit',fontWeight:700,letterSpacing:2,fontSize:12,
...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/voices').then(r=>r.json()).then(d=>{
      if(d.keyOk && d.voices?.length) {
        setVoices(d.voices); setSelectedVoice(d.voices[0].voice_id); setKeyStatus('ok')
  ...
  // STEP 2: Auto-transcribe video
  const runTranscribe = async () => {
    setError(''); setLoading(true); setLoadingMsg('Extracting audio from video...')
    setProgress(10)
    try {
      const fd = new FormData()
      fd.append('video', videoFile, videoFile.name)

      const progInt = setInterval(()=>setProgress(p => Math.min((p||0)+5, 85)), 1500)
      setTimeout(()=>setLoadingMsg('Whisper is transcribing your speech...'), 2000)
  ...
  const runMerge = async () => {
    setError(''); setLoading(true); setLoadingMsg('Merging AI voice into your video...')
    setProgress(5)
    try {
      const fd = new FormData()
      fd.append('video', videoFile, videoFile.name)
  ...
  const reset = () => {
    ...
    setCleanScript(''); setAudioBlob(null); setAudioUrl(null); setFinalVideoUrl(null)
    setError('')
  }
    ...
    <>
      ...
      <div style={{minHeight:'100vh',display:'flex',flexDirection:'column',position:'relative'}}>
        ...
        <main style={{flex:1,padding:'24px',maxWidth:680,margin:'0 auto',width:'100%',position:'relative',zIndex:1}}>
          ...
          {step==='upload' && (
            <div className="fadeUp">
              ...
              <div className="drop-zone" onDrop={onDrop} onDragOver={e=>e.preventDefault()} onClick={()=>fileRef.current.click()}
                ...
                <div style={{fontSize:10,color:'#2a2a2a'}}>MP4, MOV, WEBM · up to 200MB</div>
              ...
              <div style={{marginTop:18,padding:14,background:'#090909',borderRadius:10,border:'1px solid #0f0f0f'}}>
                ...
                {[
                  ['1','Whisper auto-transcribes your speech',''],
                  ['2','Claude cleans grammar & removes fillers',''],
                  ['3','ElevenLabs generates clean AI voice',''],
                  ['4','FFmpeg merges new audio into video','🎉'],
                ].map(([n,t,e])=>(
                  <div key={n} style={{display:'flex',gap:10,alignItems:'center',marginBottom:7}}>
                    <span style={{color:'#00e5a0',fontSize:10,width:12}}>{n}.</span>
                    <span style={{fontSize:11,color:'#444',flex:1}}>{t}</span>
                    {e && <span>{e}</span>}
                  </div>
                ))}
              </div>
          ...
          {step==='transcribe' && (
            <div className="fadeUp">
              ...
              <p style={{color:'#444',fontSize:11,marginBottom:18}}>Whisper will extract your speech automatically — no typing needed</p>
              <Card>
              ...
              <div style={{marginBottom:14}}>
                ...
                <div style={{fontSize:9,color:'#1e1e1e',marginTop:4}}>Auto-translate the cleaned script to this language</div>
              </div>
              {loading
                ? <LoadingPanel msg={loadingMsg} progress={progress}/>
                : <Btn onClick={runTranscribe}>▶ AUTO-EXTRACT TRANSCRIPT</Btn>
              }
            </div>
          )}
          ...
          {step==='enhance' && (
            <div className="fadeUp">
              ...
              <p style={{color:'#444',fontSize:11,marginBottom:18}}>Claude removes fillers, fixes grammar, and translates if needed</p>
              <Card>
                <Lbl>WHISPER TRANSCRIPT (auto-extracted)</Lbl>
                <div style={{fontSize:12,color:'#888',lineHeight:1.8,fontStyle:'italic',maxHeight:160,overflowY:'auto'}}>"{rawText}"</div>
              </Card>
              {loading
                ? <LoadingPanel msg={loadingMsg}/>
          ...
          {step==='voice' && (
            <div className="fadeUp">
              ...
              <p style={{color:'#444',fontSize:11,marginBottom:18}}>Pick the voice that will replace yours</p>
              <Card accent>
                <div style={{display:'flex',gap:8,marginBottom:10}}>
                  <Tag>✓ CLAUDE ENHANCED</Tag>
                  {targetLang!=='en'&&<Tag color='#00b8ff'>{LANGS.find(l=>l.code===targetLang)?.label}</Tag>}
                </div>
                <div style={{fontSize:12,color:'#aaa',lineHeight:1.8,maxHeight:160,overflowY:'auto'}}>{cleanScript}</div>
              </Card>
              <Lbl>SELECT VOICE</Lbl>
              <div style={{border:'1px solid #111',borderRadius:10,overflow:'hidden',marginBottom:14,maxHeight:260,overflowY:'auto'}}>
                {voices.map((v,i)=>(
          ...
          {step==='merge' && (
            <div className="fadeUp">
              <h2 style={{fontFamily:"'Bebas Neue'",fontSize:34,letterSpacing:3,color:'#fff',margin:'0 0 4px'}}>MERGE INTO VIDEO</h2>
              <p style={{color:'#444',fontSize:11,marginBottom:18}}>Preview the AI voice, then merge into your video</p>
              <Card accent>
                <Lbl>AI VOICE PREVIEW</Lbl>
                {audioUrl && <audio src={audioUrl} controls/>}
              </Card>
              <Card>
                <div style={{display:'flex',gap:10,alignItems:'center',fontSize:11}}>
                  <span style={{fontSize:18}}>🎬</span>
                  <div style={{color:'#888'}}>{videoFile?.name}</div>
                  <span style={{color:'#333',marginLeft:'auto'}}>+</span>
                  <span style={{fontSize:18}}>🔊</span>
                  <div style={{color:'#00e5a0'}}>AI voice</div>
              ...
              {loading
                ? <LoadingPanel msg={loadingMsg} progress={progress}/>
                : <Btn onClick={runMerge}>🎬 MERGE INTO FINAL VIDEO →</Btn>
              }
            </div>
          )}
        ...
        <footer style={{padding:'12px 24px',borderTop:'1px solid #0a0a0a',display:'flex',justifyContent:'space-between',zIndex:1,position:'relative'}}>
          <div style={{fontSize:8,color:'#181818'}}>VOICECRAFT v3 · WHISPER + CLAUDE + ELEVENLABS + FFMPEG</div>
